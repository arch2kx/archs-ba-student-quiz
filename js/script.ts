...
interface QuizQuestion extends Student {
...
interface QuizSettings {
    ...
    numQuestions: number;
    answerType: 'multiple-choice' | 'type-in';
    studentOrder: 'random' | 'popular' | 'unpopular';
    timerEnabled: boolean;
    timerDuration: number;
}

interface UserAnswer {
    student: QuizQuestion;
...
let quizSettings: QuizSettings = {} as QuizSettings;
...
function getStudentAge(student: Student): number {
...
function getCorrectAnswer(student: Student, type: QuestionType): string | number {
    switch (type) {
...
function getDistractors(correct: string | number, type: QuestionType): (string | number)[] {
    switch (type) {
        case 'age': {
            const age = correct as number;
            const opts = new Set<number>();
            while (opts.size < 3) {
                const rand = age + Math.floor(Math.random() * 7) - 3;
                if (rand > 0 && rand <= 25 && rand !== age) opts.add(rand);
            }
            return [...opts];
        }
        case 'name':
...
function studentHasField(student: Student, type: QuestionType): boolean {
    switch (type) {
...
function answersMatch(user: string | number, correct: string | number, type: QuestionType): boolean {
    if (type === 'age') return Number(user) === Number(correct);
    if (type === 'height') {
        const nh = (s: string) => s.toLowerCase().replace(/\s+/g, '').replace('cm', '');
        return nh(String(user)) === nh(String(correct));
    }
    if (type === 'birthday') {
        const nb = (s: string) => s.toLowerCase().replace(/\s+/g, '').replace(/(st|nd|rd|th)/g, '');
...
function toggleTheme(): void {
    ...
    const themeIcon = themeToggle.querySelector('.theme-icon') as HTMLImageElement;
...
async function fetchStudentData(): Promise<boolean> {
    ...
    try {
        ...
        const studentsArray: Student[] = Array.isArray(data)
            ? data
            : Object.values(data as Record<string, Student>);
        ...
        console.error(error);
        loadingDiv.classList.add('hidden');
        errorMessage.textContent = `Error loading student data: ${(error as Error).message}. Please try again.`;
...
async function startQuiz(): Promise<void> {
    const timerCustomInput = document.getElementById('timer-custom') as HTMLInputElement;
    const setupScreen = document.getElementById('setup-screen')!;
    const quizScreen = document.getElementById('quiz-screen')!;

    const selectedPreset = document.querySelector<HTMLInputElement>('input[name="timer-preset"]:checked');
    ...
    const numQuestionsVal = (document.getElementById('num-questions') as HTMLSelectElement).value;
    let numQuestions: number;
    if (numQuestionsVal === 'custom') {
        const custom = parseInt((document.getElementById('custom-questions') as HTMLInputElement).value);
    ...
    quizSettings = {
        quizType: (document.getElementById('quiz-type') as HTMLSelectElement).value as QuizSettings['quizType'],
        numQuestions,
        answerType: (document.getElementById('answer-type') as HTMLSelectElement).value as QuizSettings['answerType'],
        studentOrder: (document.getElementById('student-order') as HTMLSelectElement).value as QuizSettings['studentOrder'],
        timerEnabled: (document.getElementById('timer-enabled') as HTMLInputElement).checked,
        timerDuration,
    };
    ...
    (document.getElementById('logo') as HTMLElement).classList.add('hidden');
    displayQuestion();
}

function prepareQuizQuestions(): void {
    let pool = [...students];

    if (quizSettings.quizType !== 'mixed') {
        pool = pool.filter(s => studentHasField(s, quizSettings.quizType as QuestionType));
    }

    if (quizSettings.studentOrder === 'unpopular') pool = shuffleArray(pool).reverse();
    else pool = shuffleArray(pool);
    ...
    quizQuestions = selected.map(student => {
        ...
        if (quizSettings.quizType === 'mixed') {
            ...
            questionType = quizSettings.quizType as QuestionType;
        }
...
function displayQuestion(): void {
    ...
    const studentImage = document.getElementById('student-image') as HTMLImageElement;
    const studentNameEl = document.getElementById('student-name')!;
    const questionText = document.querySelector<HTMLElement>('.question h3')!;
...
function displayMultipleChoice(correctAnswer: string | number, type: QuestionType): void {
...
function displayTypeIn(correctAnswer: string | number, type: QuestionType): void {
    ...
    const inputIds: Record<QuestionType, string> = {
    ...
    if (type === 'age') { input.min = '1'; input.max = '25'; }
    ...
    submit.addEventListener('click', () => {
        ...
        if (!raw) return;
        const val: string | number = type === 'age' ? parseInt(raw) : raw;
        if (type === 'age' && isNaN(val as number)) return;
...
function checkAnswer(userAnswer: string | number, correctAnswer: string | number, type: QuestionType): void {
    stopTimer();
    ...
    document.querySelectorAll<HTMLButtonElement>('.answer-btn').forEach(btn => { btn.disabled = true; });
    const inputIds: Record<QuestionType, string> = {
        age: 'age-input', name: 'name-input', academy: 'academy-input',
        birthday: 'birthday-input', height: 'height-input', voice: 'voice-input',
        role: 'role-input', damageType: 'damagetype-input',
    ...
    const inputField = document.getElementById(inputIds[type]) as HTMLInputElement | null;
    if (inputField) inputField.disabled = true;
    document.querySelectorAll<HTMLButtonElement>('.submit-answer-btn').forEach(btn => { btn.disabled = true; });
...
function resetQuiz(): void {
    ...
    (document.getElementById('logo') as HTMLElement).classList.remove('hidden');
    currentQuestionIndex = 0;
    score = 0;
    userAnswers = [];
    quizQuestions = [];
...
function startTimer(): void {
    ...
    timerInterval = setInterval(() => {
        ...
        if (timeRemaining <= 0) { stopTimer(); handleTimeout(); }
    }, 1000);
}
...
function handleTimeout(): void {
    ...
    document.querySelectorAll<HTMLElement>('.answer-btn, .type-in-input, .submit-answer-btn')
...
function openShareModal(): void {
    ...
    if (!shareText || !shareModal) return;
    const percentage = Math.round((score / quizQuestions.length) * 100);
    const label = QUESTION_LABELS[quizSettings.quizType as QuestionType] ?? quizSettings.quizType;
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    const themeToggle   = document.getElementById('theme-toggle');
    const timerCheckbox = document.getElementById('timer-enabled') as HTMLInputElement | null;
    const timerSettings = document.getElementById('timer-settings');
    const timerCustom   = document.getElementById('timer-custom') as HTMLInputElement | null;
    const timerPresets  = document.querySelectorAll<HTMLInputElement>('input[name="timer-preset"]');
    ...
    if (timerCheckbox && timerSettings) {
        timerCheckbox.addEventListener('change', e => {
            timerSettings.style.display = (e.target as HTMLInputElement).checked ? 'block' : 'none';
        });
    }
    ...
    timerPresets.forEach(radio => {
        radio.addEventListener('change', e => {
            ...
            if (timerCustom) {
                ...
                if (val === 'custom') timerCustom.focus();
            }
        });
    });
    ...
    const numQSelect    = document.getElementById('num-questions') as HTMLSelectElement | null;
    const customWrapper = document.getElementById('custom-questions-wrapper');
    const customInput   = document.getElementById('custom-questions') as HTMLInputElement | null;

    numQSelect?.addEventListener('change', e => {
        const isCustom = (e.target as HTMLSelectElement).value === 'custom';
        if (customWrapper) customWrapper.style.display = isCustom ? 'flex' : 'none';
